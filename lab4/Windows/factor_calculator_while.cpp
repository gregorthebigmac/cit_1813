// This has the same story as the for loop version. I originally wrote it for Linux and made it cross-platform, but when I ported it
// to Windows via Visual Studio, it threw a bunch of errors, so I created a new version for Windows only, and left the original alone.
// If you want to see the original, you will find it at https://github.com/gregorthebigmac/cit_1813 and look in lab4/
...
vector<int> calculate_factors(int number_to_factor);
...
vector<int> calculate_factors(int number_to_factor) {
	vector<int> factor_list;
	int loop_counter = 1;
	while (loop_counter < number_to_factor) {
		if (number_to_factor % loop_counter == 0)
			factor_list.push_back(loop_counter);
		loop_counter++;
	}
	return factor_list;
}
